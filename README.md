...
|------------------------|-----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| Инвертированный индекс  | Поиск по текстовым полям (названия, описание товаров)      | Ускоряет поиск по текстовым полям, обеспечивает быстрый доступ к товарам по ключевым словам.                             |
| Бинарный поиск          | Фильтрация по числовым значениям (цена, вес)              | Осуществляет быстрый поиск по отсортированным данным, уменьшает количество операций поиска.                              |
| Фасетная фильтрация     | Множественные фильтры по категориям, цвету, цене и т.д.   | Позволяет пользователям комбинировать несколько фильтров для гибкого поиска, оптимизирует работу с большими наборами данных. |
...
| **Алгоритм**           | **Область применения**                                    | **Мотивационная часть**                                                                                                  |
|------------------------|-----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| Алгоритм сортировки     | Сортировка отзывов по дате, рейтингу                      | Оптимизирует отображение отзывов, позволяя пользователям видеть самые свежие или наиболее полезные отзывы.                |
| Взвешенное среднее      | Расчет среднего рейтинга товаров                          | Учитывает количество оценок и их вес, обеспечивая справедливое распределение рейтинга товаров с учетом всех отзывов.      |
...
| **Алгоритм**                          | **Область применения**                                    | **Мотивационная часть**                                                                                                  |
|---------------------------------------|-----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| Коллаборативная фильтрация            | Рекомендация товаров на основе предпочтений пользователя   | Повышает релевантность рекомендаций за счет анализа покупок или просмотров пользователя и других пользователей с похожими предпочтениями. |
| Item-based Collaborative Filtering    | Рекомендация товаров на основе их схожести                 | Рекомендует пользователям товары, похожие на те, что они уже покупали или оценивали, увеличивая вероятность покупки.      |
...
| **Алгоритм**                | **Область применения**                                    | **Мотивационная часть**                                                                                                  |
|-----------------------------|-----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| Алгоритмы валидации          | Проверка корректности данных заказа                       | Гарантирует, что данные пользователя корректны и готовые для обработки, снижая вероятность ошибок при оформлении заказа.  |
| Алгоритмы создания транзакций | Генерация уникальных идентификаторов для заказов и транзакций | Обеспечивает целостность данных при оформлении заказа и оплате, предотвращая дублирование и ошибки.                      |
...
| **Алгоритм**                | **Область применения**                                    | **Мотивационная часть**                                                                                                  |
|-----------------------------|-----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| Алгоритмы сопоставления      | Сравнение атрибутов товаров (цена, размеры, цвет и т.д.)  | Упрощает процесс сравнения товаров для пользователей, помогая им принимать решения на основе ключевых характеристик.      |

---

### Управление учетной записью и заказами

| **Алгоритм**                | **Область применения**                                    | **Мотивационная часть**                                                                                                  |
|-----------------------------|-----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| CRUD (Create, Read, Update, Delete) | Основные операции для управления данными учетных записей и заказов | Обеспечивает надежное управление данными пользователей и заказов, поддерживая актуальность информации в системе.          |
| Алгоритмы аутентификации     | Проверка учетных данных, хэширование паролей              | Обеспечивает безопасность учетных записей, предотвращая несанкционированный доступ.                                       |
