...
Для надежности системы балансировки сделаем многоуровневую архитектуру с неколькими уровнями балансировщиков.

  1. Входной уровень
    Будем использовать 2 (лучше 3) балансировщика NGINX для обеспечения отказоустойчивости.Через этот уровень проходят все внешние запросы к приложению, для них можно использовать Round Robin или Least Connections. Также на этом уровне будет проходить терминация SSL и начальное распределение нагрузки между внутренними балансировщиками
 
    Задачи:
    - Терминация SSL
    - Начальное распрделение нагрузки между внутренними балансировщиками
  
  2. Внутренний уровень балансировки
    Будем использовать 2 (лучше 3) Istio для распределения запросов между микросервисами. Для внутренней балансировки используем Dynamic Load Balancing или Weighted Round Robin в зависимости от производительности разных микросервисов.
